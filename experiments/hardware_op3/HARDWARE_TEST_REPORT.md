...
The benchmark scripts record timestamp, repository commit at measurement time (`c5fe60711f615e71aa73f54586d8b16ffac96122`), hostname, CPU, RAM, Python/OpenVINO/ROS versions, model SHA256, command, device, resolution, batch, warmup count, and iteration/duration. Process CPU is psutil's aggregate across cores (100% equals one logical core), so values above 100% indicate multicore use. Resource sampling interval was 0.2 seconds. The host also had other ROS workspace activity; system CPU is provided to show concurrent system load.
